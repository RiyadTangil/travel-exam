...
export function FileUpload({
  ...
  const uploadEndpoint = ENDPOINTS.UPLOAD;
    ...
    try {
      ...
      if (file.type.startsWith("image/") && !file.type.includes("svg")) {
        try {
          ...
          console.warn("File upload compression fallback:", compErr);
        }
      }
    ...
    <div className="w-full">
      ...
      {!previewUrl && !isUploading && (
        <div 
          onClick={handleClick}
          className={cn(
            "border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 flex flex-col items-center justify-center transition-colors",
            disabled ? "bg-muted/30 cursor-not-allowed opacity-60" : "cursor-pointer hover:bg-muted/50",
            className
          )}
        >
