...
## 0. Transaction Directions (Standard)

The application uses a unified **`debit`** / **`credit`** direction for all `client_transactions`.

| Module | Direction | Logic / Result |
|--------|-----------|----------------|
| **Invoice** | `debit` | Increases Client Due |
| **Money Receipt** | `credit` | Decreases Client Due / Increases Advance |
| **Vendor Payment** | `credit` | Decreases Vendor Due / Increases Advance |
| **Expense** | `debit` | Money leaving the business |
| **Advance Return (Client)** | `debit` | Paying back client's overpayment |
| **Advance Return (Vendor)** | `debit` | Receiving back overpayment to vendor |
| **Non-Invoice Income** | `credit` | Money entering the business |
| **Investment** | `credit` | Capital injection |
| **Balance Transfer** | `debit` (Sender) / `credit` (Receiver) | Moving funds between accounts |
| **Refund (Client)** | `credit` | Returning money to client balance |
| **Refund (Vendor)** | `debit` | Reducing our liability to vendor |
| **Opening (Due)** | `debit` | Initial debt (Client owes us / We owe vendor) |
| Opening (Advance) | `credit` | Initial credit (Client paid ahead / We paid vendor ahead) |

---

## 0.1 Transaction Charge & Net Accounting Roles

This section defines how discounts, transaction charges, and AIT affect the **Account Balance** (Bank/Cash) versus the **Entity Balance** (Client/Vendor).

| Module | Direction | Account Balance Adjustment | Entity Balance Adjustment | Example (Gross 1,000) |
|:---|:---:|:---|:---|:---|
| **Money Receipt** | In | `+ (Amount - Discount)` | `- Amount` (Client) | Receipt 1,000, Discount 100 → **Account +900**, Client -1,000 |
| **Vendor Payment** | Out | `- (Amount + Charge + AIT)` | `- Amount` (Vendor) | Pay 1,000, Charge 10, AIT 20 → **Account -1,030**, Vendor -1,000 |
| **Advance Return (Client)** | Out | `- (Amount + Charge)` | `- Amount` (Client) | Return 1,000, Charge 10 → **Account -1,010**, Client -1,000 |
| **Vendor Advance Return** | In | `+ Amount` **(Temp Logic)** | `- Amount` (Vendor) | Return 1,000, Charge 10 → **Account +1,000**, Vendor -1,000 |
| **Balance Transfer** | Out/In | Sender: `- (Amount + Charge)` / Receiver: `+ Amount` | N/A | Transfer 1,000, Charge 10 → **Sender -1,010**, **Receiver +1,000** |

> **Note on Vendor Advance Return:** Currently, the application updates the account balance with the **Gross Amount** (ignoring the transaction charge) to maintain consistency with external system reporting. This logic may be updated in the future to reflect the net amount (+ Amount - Charge).
...
| **2** | **Create** | `POST /api/non-invoice-incomes` | `non_invoice_incomes`, **`accounts.lastBalance`** += amount, **`client_transactions`** (+1, `invoiceType: NON_INVOICE_INCOME`, `direction: credit`), **`counters`** seq |
...
| **2** | **Create** | `POST /api/balance-transfer` (Zod: distinct accounts, positive `amount`, optional `transferCharge`) | **`accounts`**: from −(amount+charge), to +amount; **`account_balance_transfers`**; **`client_transactions`** ×2 (same `voucherNo`, `BALANCE_TRANSFER`, debit on sender / credit on receiver) |
...
Transfer **1,000** + charge **50** from **Bank A** to **Cash B**: **A** loses **1,050**, **B** gains **1,000**; voucher **`BT-0001`**; ledger shows debit on **A** and credit on **B**. Delete reverses those balance deltas.
...
| **2** | **Create** | `POST /api/investments` | Resolves **target company** from `NonInvoiceCompany` via `companyId` in body; **`accounts.lastBalance`** += `amount`; **`investments`**; **`client_transactions`** (`INVESTMENT`, `credit`) |
...
**UI:** Same list pattern as non-debit income (`FilterToolbar`, Ant `Table`, `TableRowActions`); modal validation mirrors **`NonInvoiceIncomeModal`**.
...
User picks **Main Bank** and April 2026 → paged debits/credits with running **`lastTotalAmount`**, voucher labels, and debit-type-derived particulars in the UI mapper.
...
| **2** | **Create** | `POST /api/expenses` (Zod: date, accountId, paymentMethod, ≥1 item with positive amounts) | **`expenses`**, **`accounts.lastBalance`**, **`client_transactions`** (debit), **`counters`** |
...
```typescript
presentBalance: number // Signed number
```

- **Positive** (`> 0`) — **Advance**: Company paid **more** than owed; vendor holds a credit (we have an asset).
- **Negative** (`< 0`) — **Due**: Company owes money **to** the vendor (liability).
- **Zero** (`= 0`) — **Settled**: No outstanding balance.

### Example

1. **Create** vendor "Omor Faruk" with opening due **5,000** → `vendors` row inserted with `presentBalance: -5000`; bill-adjustment service also fires: `bill_adjustments` +1 (`opening_balance`), `client_transactions` +1 (`direction: debit`), `counters` +1.
...
5. **Delete** attempted while balance is `-5000` → API returns 403 ("Settle balance first"); once balance is 0 → `vendors` row removed.
...
| **7** | **Total creditable** | `invoices` | Global (ignores period): Sum of (`netTotal` - `creditedAmount`) for all non-deleted invoices. |
...
- **Non debit Income**: Sum of `amount` from `non_invoice_incomes` (filtered by `date`).
- **Total Income**: **Sales** + **Service Charge** + **Refund Profit** + **Non debit Income**.
...
| **2** | **Create** | `POST /api/vendors/advance-return` | `vendor_advance_returns` +1; `vendors.presentBalance` (advance ↓ by amount); `accounts.lastBalance` +amount; `client_transactions` +1 (`VENDOR_ADVANCE_RETURN`, direction `debit`) |
...
- **Create:** `vendors.presentBalance` advance decreases by `amount` (if advance balance drops below zero, it flips to `due`). `accounts.lastBalance` increases by `amount` (money credited into the account).
...
1. **Create** ADVR-0001: `vendor_advance_returns` +1; `vendors.presentBalance` becomes `{ type: "advance", amount: 7000 }`; `accounts["Main Bank"].lastBalance` +3,000; `client_transactions` ledger row `VENDOR_ADVANCE_RETURN`, direction `debit`.
...
**Purpose:** Records a payment made **to** a vendor. Four distinct payment types control which collections are affected and how the payment is linked to debit items.
...
| **4** | **Delete** | `DELETE /api/vendors/payment/[id]` | Reverses all balance and ledger effects; for "debit"/"ticket" also restores `invoice_items.paidAmount` |
...
A general payment to a vendor not linked to any specific debit or ticket.
...
| `client_transactions` | +1 row (`VENDOR_PAYMENT`, direction `credit`) |

**Allocation (post-payment):** After creating an Overall payment, the user can open the payment view page and click **"Add debit"** to allocate the amount to specific vendor debit items. Each allocation:
...
Company pays **20,000** to vendor "Omor Faruk" (overall, no debit reference) via "Main Bank":
...
- `client_transactions` +1 (`VENDOR_PAYMENT`, `credit`, `amount: 20000`)

User then allocates 12,000 to debit INV-001 and 8,000 to INV-002:
...
Pre-payment to a vendor before any debit is raised. Increases vendor's advance balance.
...
| `client_transactions` | +1 row (`VENDOR_ADVANCE`, direction `credit`) |

**Allocation (post-payment):** Same as Overall — user can later allocate advance amounts to specific debit items via the payment view page.
...
- `client_transactions` +1 (`VENDOR_ADVANCE`, `credit`)
...
#### Type 3 — Specific debit (`paymentTo: "debit"`)

Payment tied to a specific invoice. Allows selecting multiple vendor rows within that debit and setting individual amounts.

**Collections affected:**

| Collection | Change |
|-----------|--------|
| `vendor_payments` | +1 row (`paymentTo: "debit"`, with `invoiceId` and `invoiceVendors[]`) |
| `vendors` | `presentBalance` reduced by `totalAmount` |
| `accounts` | `lastBalance` −`totalAmount` |
| `client_transactions` | +1 row (`VENDOR_PAYMENT`, `credit`) |
...
debit INV-003 has two vendor cost lines:
- Omor Faruk: totalCost 8,000 | paid 0 | due 8,000
- Shahadat: totalCost 5,000 | paid 0 | due 5,000

Company pays both in one VP:
- `vendor_payments` +1 (`VP-0003`, `paymentTo: "debit"`, `invoiceId: INV-003`, `invoiceVendors: [{Omor, item1, 8000}, {Shahadat, item2, 5000}]`, `totalAmount: 13000`)
...
**Key difference from "debit":** Selection UI starts with a **vendor selector** (not an debit selector). Then it loads all non-commission ticket items for that vendor across all invoices, letting the user pick individual ticket lines.
...
| **2** | **Create** | `POST /api/vendors/payment/[id]/allocations` | `vendor_payment_allocations` +rows; `invoice_items.paidAmount` updated per vendor per debit |
...
| Route | Purpose |
|-------|---------|
| `GET /api/vendors/payment/invoices` | All invoices that have vendor cost items (for "debit" payment dropdown) |
| `GET /api/vendors/payment/invoices/[id]/vendors` | Vendor summary rows for a specific debit (totalCost / paid / due per vendor) |
...
- **Components:** `components/vendors/payment-modal.tsx`, `components/vendors/specific-debit-payment.tsx`, `components/vendors/VendorPaymentAllocateModal.tsx`
...
- **View page** (`/dashboard/vendors/payment/[id]`): Ant `Tabs` — "debit" (placeholder) and "Details". Details tab shows payment header + either line-item table (for "debit"/"ticket") or allocations table (for "overall"/"advance") with **"Add debit"** button that opens `VendorPaymentAllocateModal`.
...
**Purpose:** Records money **credited from** a client into a company **account**. The `paymentTo` field selects how that cash is applied against **invoices** and how **client** and **account** balances move. All successful creates run inside a MongoDB transaction (`withTransaction`).
...
| **4** | **Delete** | `DELETE /api/money-receipts/[id]` | Reverses client + account; reverses `invoices.creditedAmount` / `status` from `invoiceId` and from **every** `money_receipt_allocations` row; deletes allocation rows; deletes `client_transactions` with same `voucherNo` + `clientId` + `companyId` + `direction: credit`; deletes `money_receipts` |
| **5** | **Allocate (advance only)** | `POST /api/money-receipts/[id]/allocations` → `createReceiptAllocations` | `money_receipt_allocations` +rows; updates each target `invoices.creditedAmount` / `status`; `client_transactions` per allocation (`invoiceType: INVOICE`); updates MR `allocatedAmount` / `remainingAmount` |
...
| `client_transactions` | At least one row, `direction: credit`, `paymentTypeId` = account |

**`money_receipt_allocations`:** Each row links one MR to one `invoiceId` with `appliedAmount` and duplicated `voucherNo`. Used whenever payment is split across invoices (overall distribution, specific debit multi-row, tickets multi-row, and manual advance allocation from the receipt view).

---

### Type 1 — Overall (`paymentTo: "overall"`)

Client pays a lump sum **not** tied to a single debit at entry time. The service **auto-distributes** the paid amount across the client’s open invoices (newest `createdAt` first) until the cash runs out or all dues are cleared.

| Collection | Change |
|-----------|--------|
| `invoices` | For each touched invoice: `creditedAmount` ↑, `status` → `partial` / `paid` |
| `money_receipt_allocations` | One row per debit slice actually applied (may be **zero** rows if the client had no due invoices — then entire amount stays unallocated on the MR) |
...
**Delete MR-0001:** allocations removed, debit `creditedAmount` restored, client and account reversed, ledger rows for voucher removed.

---

### Type 2 — Advance (`paymentTo: "advance"`)

Client **pre-pays**; nothing is applied to a specific debit at save time. **`invoiceId`** on the MR is usually empty; **`remainingAmount`** on the MR starts equal to paid amount (minus any immediate allocation if you add rows later).
...
| `client_transactions` | Standard single `credit` row (`invoiceType` reflects `ADVANCE`) |

**Downstream:** When a **new** debit is created, `invoiceService.autoApplyClientAdvanceToInvoice` finds advance MRs with `remainingAmount > 0` (oldest first), creates `money_receipt_allocations` against the new invoice, and updates MR `allocatedAmount` / `remainingAmount`.
...
Later user opens MR view and allocates **8,000** to INV-X: `money_receipt_allocations` +1; INV-X `creditedAmount` +8,000; MR `remainingAmount` → 12,000; extra `client_transactions` row(s) per allocation rules.
...
| Path | Behaviour |
|------|-----------|
| **Single invoice** | Body includes `invoiceId`; paid amount must not exceed that invoice’s **due** (`netTotal − creditedAmount`). One allocation row (or inline debit update + allocation). |
| **Multi-row UI** | Body includes `invoiceAllocations: [{ invoiceId, amount }, …]`; sum of amounts must equal paid amount; each debit validated for ownership and due cap. |

| Collection | Change |
|-----------|--------|
| `invoices` | Each targeted invoice: `creditedAmount` ↑, `status` updated |
| `money_receipt_allocations` | One row per debit slice |
| `client_transactions` | Multiple `credit` rows when several invoices (same voucher); otherwise one row |
...
Same mechanics as **invoice** type for the backend: **`invoiceAllocations`** against **non-commission** (or ticket-scoped) debit rows, with the same validation (client owns invoice, amount ≤ due, totals match). UI collects multiple ticket lines; service treats them like multi-debit allocations.
...
- Client `presentBalance` +7,000; bank account +7,000; ledger `credit` 7,000; **no** debit `creditedAmount` change.
...
| # | Operation | Trigger | Collections |
|---|-----------|---------|---------------|
| **1** | **List** | `GET /api/money-receipts/[id]/allocations` | `money_receipt_allocations` + debit headers (read) |
| **2** | **Create** | `POST /api/money-receipts/[id]/allocations` | See operation **5** in the core table above |
| **3** | **Delete** | `DELETE /api/money-receipts/[id]/allocations/[allocId]` | Removes one allocation and reverses that slice on the debit + ledger |
...
- **Receipt form:** Payment type radio drives which extra grids appear (overall vs advance vs debit rows vs tickets vs adjust info banner).
- **debit list MR column:** Resolved via **`money_receipt_allocations.voucherNo`** (not `money_receipts.invoiceId` alone) so **overall** receipts still show on debit rows.
...
**Purpose:** Records returning **excess client advance** (or paying out held advance) **out** of a selected **account**. This is the **mirror** of crediting an advance MR: cash leaves the account and the client’s `presentBalance` drops (client is less “ahead”).
...
| **2** | **Create** | `POST /api/advance-returns` + body | `advance_returns` +1; `clients` `presentBalance` **−** amount (must have sufficient advance); `accounts` `lastBalance` **−** amount; `client_transactions` +1 (`invoiceType: Money Advance Return`, `direction: debit`) |
| **3** | **Update** | `PUT /api/advance-returns/[id]` | Transaction: adjusts client + account by **delta**; updates `advance_returns` row; updates matching `client_transactions` (same `voucherNo`, `debit`) |
| **4** | **Delete** | `DELETE /api/advance-returns/[id]` | `clients` `presentBalance` **+** amount (reversed); `accounts` `lastBalance` **+** amount; `client_transactions.deleteOne` by `voucherNo` + `clientId` + `debit`; `advance_returns` removed |

### Example

Client **CL** has `presentBalance` high enough to represent **25,000** advance. Company returns **10,000** via **Bank A** (ADR-0001):

1. **Create:** `advance_returns` +1; `clients.CL.presentBalance` 25,000 → 15,000; `accounts.A.lastBalance` −10,000; `client_transactions` debit 10,000 (`Money Advance Return`).
...
|-----------|------------------|
| `invoices` | Header + `billing` summary, `netTotal`, `creditedAmount` (0 or auto-applied advance), `status` |
...
- `invoices` +1; `invoice_items` +2; vendor balance +9,000 cost; client due +15,000 (`presentBalance` convention); `client_transactions` debit row; optional MR if payment captured on same submit.
...
**Code:** `services/invoiceService.ts` (`createInvoice`, `createNonCommissionInvoice`, `listInvoices`, `listNonCommissionInvoices`, …), `app/api/invoices/*`, modals `add-debit-modal.tsx`, `add-visa-debit-modal.tsx`, `add-non-commission-modal.tsx`.

---

## 16. Air Ticket (Invoice)

**Purpose:** A specialized invoice type (`invoiceType: "air_ticket"`) for tracking the sale of Air Tickets. Contains complex margin logic based on Gross Fare, Base Fare, Commission percentages, Advance Income Tax (AIT), and specific vendor relationships.

| Type | UI / list route | Typical create API | Child collections (in addition to `invoices` + `invoice_items`) |
|------|-----------------|--------------------|-----------------------------------|
| **D — Air Ticket** | `app/dashboard/invoices-air-ticket/page.tsx` | `POST /api/invoices/air-ticket` | `invoice_items` (`product: air_ticket`), `invoice_passports`, `invoice_transports` per ticket |

**Invoice number prefix:** `AIT-####`

### Calculation Logic (Frontend & Backend Alignment)

Air Ticket margin logic separates the **Client Price** (what the client owes) from the **Purchase Price** (what the agency owes the vendor), resulting in a calculated **Profit**.

1. **AIT (Advance Income Tax):**
   - Automatically calculated as `0.3%` of the **Gross Fare**.
   - Can be deducted from the Net Commission if the `aitType` is set to `"Profit"`.

2. **Tax:**
   - Automatically calculated as **Gross Fare - Base Fare**.

3. **Commission Amount:**
   - Automatically calculated as **Base Fare × (Commission % / 100)**.

4. **Net Commission:**
   - If `aitType === "Profit"`: `Commission Amount - AIT Amount`
   - If `aitType === "Client"`: `Commission Amount` (AIT is not deducted from agency profit).

5. **Client Price:**
   - `Gross Fare - Discount Amount + Extra Fee`
   - *Note: Discount can be a flat Amount, % of Gross Fare, or % of Base Fare.*

6. **Purchase Price:**
   - `Base Fare + Tax - Net Commission - Bonus Amount + Other Expense`
   - *Note: Bonus reduces the purchase price (vendor gives agency a bonus).*

7. **Profit:**
   - `Client Price - Purchase Price - VAT`

### Example

Agency creates an AIT invoice for client **CL** with the following inputs:
- **Gross Fare:** 50,000 | **Base Fare:** 40,000
- **Commission %:** 7% | **AIT Type:** Profit

**Auto-Calculations:**
- **Tax:** 50,000 - 40,000 = 10,000
- **AIT Amount:** 50,000 × 0.003 = 150
- **Commission Amount:** 40,000 × 0.07 = 2,800
- **Net Commission:** 2,800 - 150 = 2,650
- **Client Price:** 50,000
- **Purchase Price:** 40,000 + 10,000 - 2,650 = 47,350
- **Profit:** 50,000 - 47,350 = 2,650

**Financial Impact (`POST /api/invoices/air-ticket`):**
- `invoices` +1 (`AIT-0001`, `netTotal: 50000`)
- `invoice_items` +1 (stores raw inputs like `grossFare`, `baseFare`, but avoids saving purely derived values like `aitAmount` or `tax` to prevent data drift).
- `vendors` `presentBalance` due increases by **47,350** (Purchase Price).
- `clients` `presentBalance` due increases by **50,000** (Client Price).
- `client_transactions` debit row recorded.
...
| Debit / Credit | Derived from `direction`: `"credit"` → credit, anything else → debit |
...
|-----------------------------------|-------------------|
| `transactionType: "debit"` | `debit <voucherNo>` |
| `invoiceType: "debit"` | `Payment (Invoice)` |
...
Client "Mohammad Yeasir Arafat" has one debit (INV-0001, 15,000) and one money receipt (MR-0001, 5,000):

1. **GET** `/api/reports/client-ledger?clientId=<id>` →
   - Row 1: `debit INV-0001`, debit 15,000, balance Due 15,000
...
## 18. Vendor Ledger (report)

**Purpose:** Statement of all cost and payment movements for a single vendor, combining two sources: debit costs from `invoice_items` (grouped per invoice) and payment/return entries from `client_transactions`.
...
| Ledger impact | `direction: "debit"` → **Debit** (we paid); `direction: "credit"` → **Credit** (vendor returned advance) |

> `direction: "debit"` rows are excluded from Stream B to avoid double-counting (those costs are sourced from `invoice_items`).
...
## 19. Users and Roles (configuration)
...
4. **UI Access:** Yeasir navigates to Invoices. The "Add debit" button in `FilterToolbar` is automatically disabled (no `create` perm). In the table, `TableRowActions` shows "Edit" enabled but "Delete" disabled.
...
**Total operations: 5** (List, Create, Update, Delete, Toggle Status).

**Purpose:** Centralized hub for managing clients, their financial history, related documents (passports), and ledgers. Built with a strict **MVC** architecture to ensure consistency between the management dashboard and other modules (like Invoices/Money Receipts) that need to select or display client information.

| # | Operation | Trigger | Collections / notes |
|---|-----------|---------|---------------------|
| **1** | **List** | `GET /api/clients-manager?search&categoryId&userId&status&page&limit` | `clients` (read, paginated, backend filtered by name/phone/email, category, and assigned user) |
| **2** | **Create** | `POST /api/clients-manager` | `clients` insert; generates `uniqueId` (CL-xxxx); saves `openingBalanceAmount` and `creditLimit` |
| **3** | **Update** | `PUT /api/clients-manager/[id]` | Atomic update of client profile; `presentBalance` is managed by transactions and cannot be edited directly |
| **4** | **Delete** | `DELETE /api/clients-manager/[id]` | **Industry Standard:** Blocked if client has existing invoices or transactions to preserve financial integrity |
| **5** | **Toggle Status** | `PATCH /api/clients-manager/[id]` | Quickly activate/deactivate a client from the table view |

### Key Technical Features

1. **MVC Architecture (Users & Clients):**
   - **Users:** Unified logic for `/api/users` (selection) and `/api/configuration/users` (management) via `userService.ts` and `userController.ts`.
   - **Clients:** Business logic isolated in `clientsService.ts`, exposing clean interfaces to controllers and API routes.

2. **Advanced UI Components:**
   - **UserSelection:** Infinite-scrolling dropdown for selecting assigned users in the filter toolbar, following the `CategorySelection` pattern.
   - **TableRowActions:** Standardized row actions with built-in confirmation modals and async loading states (loader stays in modal during deletion).
   - **Hybrid Ledger:** The `ClientLedgerPage` component works as both a standalone report and a tab in the client details view, accepting either a `clientId` prop or a URL query parameter.

3. **Data Integrity & UX:**
   - **Global Input Protection:** The base `Input` component (type="number") automatically blocks invalid characters ("e", "+", "-") and negative values, ensuring data quality across all forms.
   - **Normalized Models:** Removed redundant `clientName` and `accountName` from `MoneyReceipt` and `ClientTransaction` models. Data is now dynamically populated via `$lookup`, ensuring names are always up-to-date.

4. **Client Details 360° View:**
   - **Tabs:** Details, Invoices, Payments, Passports, and Ledger.
   - **Integrated Passports:** The `Passport` tab reuses the global `PassportTable` but auto-filters it to the specific client, allowing for full CRUD operations within the client context.

### Example

1. **Setup:** Admin creates a "Corporate" client "Travel Corp" with an opening balance of 50,000 (Advance).
2. **Management:** Assigned user "Yeasir" filters the client list by "Corporate" category and sees "Travel Corp" with its unique ID (e.g., CL-0042).
3. **Transaction:** An debit is raised for "Travel Corp". The `presentBalance` in the `clients` collection is automatically adjusted.
4. **Investigation:** Admin clicks the client's name to open the **Details View**. They switch to the **Ledger** tab to see a chronological statement of all payments and invoices.
5. **Documents:** Admin switches to the **Passports** tab and adds a new passport for a traveler belonging to "Travel Corp".
6. **Safety:** A user tries to delete "Travel Corp". The backend `deleteClient` logic detects the debit and blocks the deletion with a clear message: *"Cannot delete client with existing invoices"*.

**Code Paths:**
- **Models:** `models/client.ts`, `models/money-receipt.ts`, `models/client-transaction.ts`
- **Controllers:** `controllers/clientsManagerController.ts`, `controllers/userController.ts`
- **Services:** `services/clientsService.ts`, `services/userService.ts`
- **UI Pages:** `app/dashboard/clients-manager/page.tsx` (List), `app/dashboard/clients-manager/[id]/page.tsx` (Details)
- **UI Components:** `components/clients/add-client-modal.tsx`, `components/shared/user-selection.tsx`, `components/clients/client-passports-tab.tsx`



## 21 roles ;
Updated Platform Roles
PLATFORM_SUPER_ADMIN ➡️ "P_SADMIN"
PLATFORM_ADMIN ➡️ "P_ADMIN"
PLATFORM_SUPPORT ➡️ "P_SUPP"
PLATFORM_MARKETING ➡️ "P_MKTG"
PLATFORM_DEV ➡️ "P_DEV"
Updated Company Roles
COMPANY_ADMIN ➡️ "C_ADMIN"
COMPANY_ACCOUNTANT ➡️ "C_ACC"
COMPANY_AGENT ➡️ "C_AGENT"
COMPANY_EMPLOYEE ➡️ "C_EMP"

## airport and airlines relation
Passenger
   ↓
Ticket
   ↓
Flight Segment
   ...
   https://nidhi.tourism.gov.in/home/directory?categoryCode=01&category_name=Accommodation%20Unit
